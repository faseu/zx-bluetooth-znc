...
export async function initBluetooth({
    ...
    onBluetoothDeviceFound(async ({devices}) => {
        ...
        // todo
        // bluetoothDevices = devicesGetArr.filter(item => devicesNameArr.includes(item.name))
...
export async function connectBluetooth({
    ...
    if (!devicesArr.length) {
        ...
        if (devicesArr.length) {
            uni.setStorageSync(devicesName, {deviceId, devicesArr})
        }
 ...
 * valueChangeCb 监听的蓝牙设备传值时的回调函数 {deviceId: String, serviceId: String, characteristicId: String, value: ArrayBuffer }
 */
export async function autoConnectBluetooth({
                                               connectionStateChangeCb,
                                               devicesNameArr,
                                               valueChangeCb,
                                               property = 'notify',
