<template>
  <div class="village_mode">
    ...
    <view class="wrap-header flex-between alignment">
      <view class="">
        <Bluetooth ref="bluetooth" @connectionStateChange="connectionStateChange" autoConnectDeviceName="HLK-V50" />
      </view>
      <view class="reconnectBtn" @click="reconnectBluetooth" style="color: #323848">重新连接</view>
    </view>
  </div>
</template>
    ...
    methods: {
      ...
      setContent(command) {
        ...
        this.order_items.map((item) => {
          command.goods(item.goods_name, item.count + (item.flag == 0 ? '千克' : '件 '));
          command.setTexta('');
        });
        command.setTexta('-------------------------------');
